...
const FogBackground = () => {
  ...
  useEffect(() => {
    ...
    // 创建柔和的烟雾纹理 - 调整了透明度使其更明显
    const gradient = puffCtx.createRadialGradient(puffSize/2, puffSize/2, 0, puffSize/2, puffSize/2, puffSize/2);
    gradient.addColorStop(0, 'rgba(200, 230, 255, 0.25)'); // 中心亮度增加
    gradient.addColorStop(0.4, 'rgba(150, 200, 255, 0.1)'); // 边缘浓度增加
    ...
    class Particle {
      constructor(x, y, type = 'ambient') {
        ...
        this.growth = Math.random() * 0.8 + 0.2; // 扩散速度增加
        this.life = 0;
        this.maxLife = Math.random() * 250 + 150; // 存活更久
        
        // 物理属性
        if (type === 'mouse') {
          // 鼠标产生的雾气
          const angle = Math.random() * Math.PI * 2;
          const speed = Math.random() * 3; // 速度稍微加快
          this.vx = Math.cos(angle) * speed;
          this.vy = Math.sin(angle) * speed - 1.5; // 上升动力更强
          this.maxLife = 120;
          this.size = 40; 
          this.alphaMult = 2.0; // 鼠标产生的烟雾更亮
        } else {
          // 环境雾气
          this.vx = (Math.random() - 0.5) * 0.8;
          this.vy = -(Math.random() * 0.8 + 0.4); // 上升速度增加
          this.alphaMult = 1.0;
        }
      }
      ...
      draw(ctx) {
        ...
        let alpha = 0;
        if (this.life < 40) {
          alpha = (this.life / 40) * this.alphaMult;
        } else if (this.life > this.maxLife - 80) {
          alpha = ((this.maxLife - this.life) / 80) * this.alphaMult;
        } else {
